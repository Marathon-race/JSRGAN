...
def is_image_file(filename):  # 通过后缀检查是否为图片文件
    return any(filename.endswith(extension) for extension in ['.png', '.jpg', '.jpeg', '.PNG', '.JPG', '.JPEG'])


def calculate_valid_crop_size(crop_size, upscale_factor):  # 实际有效的图片区域范围
...
class TrainDatasetFromFolder(Dataset):  # 从文件夹获取训练集
...
class ValDatasetFromFolder(Dataset):  # 验证集
...
class TestDatasetFromFolder(Dataset):  # 测试集
    def __init__(self, dataset_dir, upscale_factor):
